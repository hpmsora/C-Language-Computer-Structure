 ...
 * f3d_gyro.c
 *
 * Contains functions to initialize and communicate with the gyroscope
 *
 * Author: Max McLennan (mpmclenn)
 *         Zach Bray    (zhbray)
 * Date Created: 2/18/16
 * Last Modified by: Max McLennan
 * Date Last Modified: 2/23/16
 * Assignment: Lab 6
...
void f3d_gyro_interface_init() {
  ...
  // Use alternate functions for A pins
...
void f3d_gyro_write(uint8_t* pBuffer, uint8_t WriteAddr, uint16_t NumByteToWrite) {
  ...
  while(NumByteToWrite > 0x00) {
    // send the current byte in the pBuffer, ignore result
